    ...
    Object.keys(clusterData).forEach((cluster) => {
      ...
      clusterAverages[cluster] = Object.keys(clusterAverages[cluster]).map(
        (key) => ({
          ...
          average:
            key === "food" || key === "total_consume"
              ? avgStats
                ? (avgStats[`${key}__avg`] || 0)  /10
                : 0
              : avgStats
              ? avgStats[`${key}__avg`] || 0
              : 0,
        })
      );
...
export default ConsumptionCluster;
